...
//voter profiles whether with queries or none
router.get('/', (req,res)=>{
	//store the values of the queries
  const active = req.query.active;
	const verified = req.query.verified;
	let voterPromise;
	//if both queries are undefined, get all the voters
	if(typeof(active) === "undefined" && typeof(verified) === "undefined"){
		voterPromise = User.find();
	}
	//if we have values for BOTH queries and they are strings
		...
		if((active === "true" || active === "false") && (verified === "true" || verified === "false")){
			voterPromise = User.find({isActive: active, verified: verified});
		}
		else{
...
//View a single voter account/profile
...
//View a single voter schedules
router.get('/:id/schedules', (req, res) => {
	Schedule.find({voter: req.params.id}).populate('bookings.organization')
...
//create a new voter profile/account
router.post('/', (req, res)=>{
  ...
  return User.find({userName: req.body.userName})
	...
	.then(newVoter => res.status(201).json(newVoter))
  .catch(err => {
...
router.post('/:id/schedules', (req, res)=>{
	...
	if(!(req.params.id === req.body.voter)){
		const message = `The request path ID ${req.params.id} and voter ID ${req.body.voter} should match.`;
		console.error(message);
		return res.status(400).send(message);
	}
	//store the required properties in an array
	const requiredFields = ['schedType', 'startingDate', 'endingDate', 'dayOfWeek', 'time','voter', 'bookings'];
	//use for loop to check if all required properties are in the req body
	for(let i=0; i<requiredFields.length; i++){
		const field = requiredFields[i];
		if(!(field in req.body)){
	...
	Schedule.create({
		...
		time: req.body.time,
		voterFirstName: req.body.firstName,
    voterLastName: req.body.lastName,
...
//update a specific voter account/profile
...
//update the schedule settings of a voter
router.put('/:id/schedules', (req, res)=>{
	...
	Schedule.findOneAndUpdate({voter: req.params.id}, {$set: toUpdate}, {new: true})
	.then(()=>{
		return Schedule.find({voter: req.params.id})
			.then(data => res.status(200).json(data));
	})
	.catch(err => {
		console.log(err);
...
//disable a specific voter profile/account by setting isActive to false
