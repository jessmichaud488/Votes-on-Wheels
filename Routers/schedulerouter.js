...
router.get('/', (req,res)=>{
	...
	if(typeof(active) === "undefined"){
		schedulePromise = Schedule.find().populate('driver');
	}
	//if the query has a value
	else if(typeof(active) === "string"){
		//if value is true, show schedules that are active
		if(active === "true"){
			schedulePromise = Schedule.find({endingDate: {$gte: today}, startingDate: {$lte: finalDate}}).populate('driver');
		}
		//else show the past schedules
		else if(active === "false") {
			schedulePromise = Schedule.find({endingDate: {$lt: today}}).populate('driver');
		}
		//everything else
		else {
...
//get the schedule of a specific driver
router.get('/:id', (req,res)=>{
	Schedule.findOne({driver: req.params.id}).populate('driver')
		.then(data => {
			let today = moment();
			...
			for(let bookDateKey in availBookDates){
				const start = moment(data.startingDate);
...
//book an available date from a driver schedule
router.put('/:schedId', (req, res) => {
	const dateToBook = {};
	//store the required properties in an array
	const requiredFields = ['date', 'voterName'];
